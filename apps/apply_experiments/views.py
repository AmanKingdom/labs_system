...
def personal_homepage(request):
    context = {
        ...
        'user': Teacher.objects.get(account=request.session['user_account']),
        'course_amount': None,
        'term': Term.objects.all()[0]
    }
    context['course_amount'] = len(Course.objects.filter(teachers__account=context['user'].account))
